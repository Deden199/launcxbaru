...
function parseArgs(argv: string[]): Args {
  const out: Args = {}#!/usr/bin/env ts-node

/**
 * Cleanup Order.metadata & loanedAt untuk tanggal WIB
 *
 * Contoh:
 *  npx ts-node scripts/cleanup-order-metadata-loan.ts --date=2025-09-25 --dryRun
 *  npx ts-node scripts/cleanup-order-metadata-loan.ts --date=2025-09-25 --confirm
 *
 * Opsi:
  ...
  for (let i = 0; i < candidates.length; i += BATCH) {
    ...
    const ops = batch.map((o) =>
      prisma.order.update({
        where: { id: o.id },
        data: {
          // NOTE: untuk MongoDB + Prisma v5.x, tulis null langsung (bukan Prisma.JsonNull)
          metadata: null,
          loanedAt: null,
        },
        select: { id: true },
