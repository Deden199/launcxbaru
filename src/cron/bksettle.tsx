  ...
  cron.schedule(
    ...
    async () => {
      try {
        ...
        logger.error('[BkSettleCron] Unexpected error:', err)
        try {
          if (config.api.telegram.adminChannel) {
            await sendTelegramMessage(
              config.api.telegram.adminChannel,
              `[BkSettleCron] Fatal error: ${err instanceof Error ? err.message : err}`
            )
          }
        } catch (telegramErr) {
          logger.error('[BkSettleCron] Failed to send Telegram alert:', telegramErr)
        }
      }
    },
